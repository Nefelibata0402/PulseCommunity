...
func (c *Callbacks) Initialize(db *gorm.DB) error {
	...
	err = db.Callback().Query().Before("*").Register("prometheus_raw_before", c.Before())
	if err != nil {
		return err
	}
	...
	err = db.Callback().Update().After("*").Register("prometheus_delete_after", c.After("DELETE"))
	if err != nil {
		return err
	}
	...
	err = db.Callback().Update().After("*").Register("prometheus_row_after", c.After("ROW"))
	return err
}
...
func (c *Callbacks) Before() func(db *gorm.DB) {
	return func(db *gorm.DB) {
		...
		fmt.Printf("Starting %s operation on table %s\n", db.Statement.Schema.Name, db.Statement.Table)
	}
}
...
func (c *Callbacks) After(typ string) func(db *gorm.DB) {
	return func(db *gorm.DB) {
		...
		if ok {
			...
			fmt.Printf("Finished %s operation on table %s, duration: %d ms\n", typ, db.Statement.Table, duration)
		}
	}
}

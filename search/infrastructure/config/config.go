...
func InitConfig() *Config {
	...
	conf.Viper.AddConfigPath(workDir + "/search/infrastructure/config")
	err := conf.Viper.ReadInConfig()
	if err != nil {
		zap.L().Error("InitConfig 读取配置文件失败")
		return nil
	}
	...
	conf.InitMongoDBConfig()
	conf.InitElasticSearchConfig()
	return conf
}
...
func (c *Config) InitMongoDBConfig() {
	mongoDB := &MongoDBConfig{
		...
		Port:     c.Viper.GetInt("mongodb.port"),
		Db:       c.Viper.GetString("mongodb.db"),
	}
	c.MongoDBConfig = mongoDB
}
...
func (c *Config) InitElasticSearchConfig() {
	es := &ElasticSearchConfig{
		Username: c.Viper.GetString("elasticsearch.username"),
		Password: c.Viper.GetString("elasticsearch.password"),
		Host:     c.Viper.GetString("elasticsearch.host"),
		Port:     c.Viper.GetInt("elasticsearch.port"),
	}
	c.ElasticSearchConfig = es
}
